...
export default async function handler(req, res) {
  try {
    ...
    const finalSubject = 'You are invited to join HyperBrain';
    const finalHtml = `<h1>Join HyperBrain</h1><p>You have been invited to study together. Click <a href="${link || '#'}">here</a> to sign up.</p>`;
    ...
    console.error("Internal server error in invite handler:", error);
    return res.status(500).json({ success: false, message: "Unable to send email. Please try again later." });
  }
}
