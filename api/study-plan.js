...
export default async function handler(req, res) {
  try {
    ...
    if (!subjectName || !topics) {
      return res.status(400).json({
        success: false,
        message: "Missing subjectName or topics parameter"
      });
    }

    const cacheKey = `study_plan_${subjectName.replace(/\s+/g, '_')}_${topics.map(t => typeof t === 'string' ? t : t.title).join("_").replace(/\s+/g, '_').substring(0, 50)}`;
    ...
    // 2. Validate usage limits (Study plans: 3/day)
    const d = new Date();
    ...
    console.error("Generate study plan API error:", error);
