...
export default async function handler(req, res) {
  try {
    ...
    const finalSubject = 'Reset Your Password';
    const finalHtml = `<h1>Password Reset Request</h1><p>Click <a href="${link || '#'}">here</a> to reset your password. Token: ${token || 'N/A'}</p>`;
    ...
    console.error("Internal server error in reset-password handler:", error);
    return res.status(500).json({ success: false, message: "Unable to send email. Please try again later." });
  }
}
