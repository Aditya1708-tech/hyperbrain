...
export default async function handler(req, res) {
  ...
  const { subjectName, topics, examDate, studyHours } = req.body;
  if (!subjectName || !topics || !examDate || !studyHours) {
    return res.status(400).json({ success: false, message: "Missing required parameters" });
  }

  const cacheKey = `studyplan_${subjectName.replace(/\s+/g, '_')}_${examDate}_${studyHours}`;
  ...
  // 2. Check limits (Study Plans: 3/day)
  const d = new Date();
  const dateStr = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`;
  const usageId = `${userId}_study_plans_${dateStr}`;
  ...
  try {
    ...
    console.error("Server API generate-study-plan failed:", err);
    // Return custom mock fallback array
    const fallback = topics.map((t, idx) => {
      ...
      return {
        ...
        tasks: [
          { text: `Review syllabus outline: ${typeof t === 'string' ? t : t.title}`, completed: false },
          { text: `Practice MCQ simulator for 10 minutes`, completed: false }
        ],
        completed: false
      };
    });
    return res.status(200).json({ success: false, message: "AI service temporarily unavailable", data: fallback });
  }
}
